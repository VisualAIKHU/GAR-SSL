...
from data_utils import load_music_duet_gt
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="GAR - MUSIC Duet")
    parser.add_argument("--model_id",     type=str,   default="Qwen/Qwen2.5-Omni-7B")
    parser.add_argument("--frame_dir",    type=str,   default="/data/subin/MUSIC/duet/test/frames")
    parser.add_argument("--audio_dir",    type=str,   default="/data/subin/MUSIC/duet/test/audio")
    parser.add_argument("--gt_path",      type=str,   default="/data/subin/metadata/music_duet.json")
    parser.add_argument("--out_root",     type=str,   default="/data/subin/GAR_CVPR26/outputs/GAR_music_duet")
    ...
    parser.add_argument("--tau_audio",    type=float, default=0.50)
    ...
    prompt_locate_A = build_prompt_locate_duet(dataset_name="MUSIC DUET")
    prompt_audio_B  = build_prompt_audio_duet(dataset_name="MUSIC DUET")
    ...
    gt_dict     = load_music_duet_gt(gt_path)
    ...
    for i, frame_file in enumerate(frame_files, 1):
        ...
        try:
            ...
            text_A   = run_chat(
                ...
                system_text="You are a multimodal assistant for audio-visual scene understanding (MUSIC Duet - Stage A).",
            ...
            text_B = run_chat(
                ...
                system_text="You are an audio classification expert (MUSIC Duet - Stage B).",
                do_sample=False, temperature=0.0, top_p=1.0
            )
            ...
            for k in range(2):
                analysis_k = run_anchor_voting_multi(
                    ...
                    lambda bb=bboxes_A[k], ac=audio_classes[k], cf=audio_confs[k], w=W, h=H:
                        build_prompt_analysis_anchor_voting(bb, ac, cf, w, h),
                    n=args.n_votes
                )
                analyses.append(analysis_k)
                av_consistencies.append(float(analysis_k.get("av_consistency", 0.0)))
                keep_flags.append(bool(analysis_k.get("keep", False)))
                role_tags_list.append(analysis_k.get("role_tags", []))
                anchor_votes_list.append(analysis_k.get("anchor_votes", []))

            # ---------- 게이팅 (τ_av=0.75, τ_aud=0.50) ----------
            do_refines = []
            for k in range(2):
            ...
            for p224 in pboxes_224:
                cv2.rectangle(pred_mask, (p224[0], p224[1]), (p224[2], p224[3]), 1, -1)
            for g224 in gboxes_224:
                cv2.rectangle(gt_mask, (g224[0], g224[1]), (g224[2], g224[3]), 1, -1)
            ...
            if len(gboxes_224) == 2 and len(pboxes_224) == 2:
                i00, i01 = iou_xyxy(_to_xyxy(pboxes_224[0]), _to_xyxy(gboxes_224[0])), iou_xyxy(_to_xyxy(pboxes_224[0]), _to_xyxy(gboxes_224[1]))
                i10, i11 = iou_xyxy(_to_xyxy(pboxes_224[1]), _to_xyxy(gboxes_224[0])), iou_xyxy(_to_xyxy(pboxes_224[1]), _to_xyxy(gboxes_224[1]))
                if (i00 + i11) >= (i01 + i10):
                    matched_mean_iou = (i00 + i11) / 2.0
                    per_source_iou   = [i00, i11]
                else:
                    matched_mean_iou = (i01 + i10) / 2.0
                    per_source_iou   = [i01, i10]
            elif len(gboxes_224) == 1 and len(pboxes_224) == 2:
                i0 = iou_xyxy(_to_xyxy(pboxes_224[0]), _to_xyxy(gboxes_224[0]))
                i1 = iou_xyxy(_to_xyxy(pboxes_224[1]), _to_xyxy(gboxes_224[0]))
                matched_mean_iou = max(i0, i1)
                per_source_iou   = [i0, None] if i0 >= i1 else [None, i1]
            ...
            per_json = {
                ...
                "analysis": [
                    {"av_consistency": round(av_consistencies[0], 3), "role_tags": role_tags_list[0],
                     "anchor_votes": anchor_votes_list[0], "keep": keep_flags[0]},
                    {"av_consistency": round(av_consistencies[1], 3), "role_tags": role_tags_list[1],
                     "anchor_votes": anchor_votes_list[1], "keep": keep_flags[1]},
                ],
